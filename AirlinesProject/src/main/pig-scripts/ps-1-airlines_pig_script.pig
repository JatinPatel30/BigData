...
LOAD_AIRPORTS = LOAD '/user/edureka_1548787/projects/project1/airports_mod.dat' using PigStorage(',')
...
LOAD_AIRLINES = LOAD '/user/edureka_1548787/projects/project1/Final_airlines' using PigStorage(',')
...
LOAD_ROUTES = LOAD '/user/edureka_1548787/projects/project1/routes.dat' using PigStorage(',')
	as (airline:chararray, airline_id:long, source_airport:chararray, source_airport_id:long,
	destination_airport:chararray, destination_airport_id:long, codeshare:chararray, stops:long, equipment:chararray);
...
------------------------------------------------------------------------------------------------------------------------------------------
SELECT_REQUIRED_COLUMN_AIRPORTS = FOREACH LOAD_AIRPORTS GENERATE name, country;
FILTER_AIRPORTS_BY_COUNTRY_INDIA = FILTER SELECT_REQUIRED_COLUMN_AIRPORTS by country == 'India';
...
------------------------------------------------------------------------------------------------------------------------------------------
SELECT_AIRLINE_REQUIRE_COLUMNS = FOREACH LOAD_AIRLINES GENERATE airline_id, name;
SELECT_ROUTE_REQUIRE_COLUMNS = FOREACH LOAD_ROUTES GENERATE airline_id, stops;
...
SELECT_AIRLINE_REQUIRE_COLUMNS = FOREACH LOAD_AIRLINES GENERATE airline_id, name;
SELECT_ROUTE_REQUIRE_COLUMNS = FOREACH LOAD_ROUTES GENERATE airline_id, codeshare;
